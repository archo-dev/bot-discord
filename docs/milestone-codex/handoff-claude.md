...
M01 à M08 sont implémentées selon les statuts détaillés ci-dessous. M09 « tickets d'équipe » est implémentée localement sur `milestone/team-tickets`, avec la migration additive `0030_team_tickets.sql`, sans migration distante ni déploiement. M10 et les cinq éléments de backlog restent validés au niveau produit, sans autorisation implicite d’implémentation.
...
M07 (sauvegarde/restauration) : migration additive `0025_config_snapshots.sql`. Snapshots canoniques versionnés de Config `general`, Automod et de la configuration Tickets M09, sérialiseurs allowlistés (jamais de secret — test anti-secret), checksum SHA-256, rétention 25/guilde. Routes admin-only `config-snapshots` (list/create/get), `/:id/diff`, `/:id/restore` (sélectif, atomique, snapshot pré-restore), `/:id/export`, `config-import/validate`+`/apply` (remap rôles/salons explicite, snapshot pré-import). Contrat shared `config-backup.ts` (zod, refs, remap, canonicalJson, diff). Panel : page « Sauvegarde » (`/backup`). Aucune écriture Gateway. Documentation `docs/config-backup.md`. **Extension Tickets locale sur `milestone/team-tickets`, sans migration remote ni déploiement.**

M08 (analytics produit privées) : migration additive `0026_privacy_analytics.sql`, taxonomie partagée stricte de six événements et dimensions bornées, contributions par guilde pseudonymisées avec HMAC journalier, agrégation/purge cron (7/180 jours) et feedback séparé (60 jours). Instrumentation Worker des installations/désinstallations, de l'onboarding M06 et des changements de modules M03. Routes admin-only `GET/PATCH /privacy` et `POST /feedback`, page panel « Confidentialité », coupe-circuit `PRODUCT_ANALYTICS_ENABLED`. Vue agrégée `GET /internal/product-metrics` sous authentification interne avec seuil de trois guildes. Documentation `docs/privacy-analytics.md`. **Migration distante appliquée et Worker/panel déployé le 2026-07-17 (version `01e33ccc-e192-42c2-a924-12dfda86dfed`, `/health` 200).**

M09 (tickets d'équipe) : migration additive `0030_team_tickets.sql`, compatibilité `status` historique, états `open/pending/closed`, priorité simple, assignation atomique, réservation durable avant REST Discord avec compensation, formulaires versionnés (5 catégories/3 champs), actions staff Discord, API panel admin-only, timeline bornée/purgée et statistiques sans contenu. La configuration tickets rejoint les snapshots M07. Sans scheduler C2.0, seuls les tickets vieillissants sont signalés ; aucun rappel actif. Documentation `docs/team-tickets.md`. **Implémenté localement sur `milestone/team-tickets`, sans migration distante ni déploiement.**
