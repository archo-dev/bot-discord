...
import { consumeInternalNonce } from "../db/queries.js";
...
const ROUTES: ReadonlyArray<[string, RegExp]> = [
  ...
  ["POST", /^\/internal\/capability-metrics$/],
...
export const internalAuthentication: MiddlewareHandler<{ Bindings: Env }> = async (c, next) => {
  ...
  if (mode !== "legacy" && hasSignature) {
    ...
    try {
      const nonceHash = await sha256Hex(`gateway-to-worker:${verified.nonce}`);
      if (!(await consumeInternalNonce(c.env.DB, "gateway-to-worker", nonceHash, verified.timestamp))) {
