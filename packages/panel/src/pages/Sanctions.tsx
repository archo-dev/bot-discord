...
function History() {
  ...
  return <div className="space-y-4">
    ...
    <Drawer open={detail !== null} onClose={() => setDetail(null)} title={detail ? `Détail de la sanction #${detail.id}` : "Détail de la sanction"}>{detail && <><dl className="grid gap-4 text-sm"><div><dt className="text-zinc-500">Raison</dt><dd className="mt-1 text-zinc-200">{detail.reason ?? "Aucune raison"}</dd></div><div><dt className="text-zinc-500">Expiration</dt><dd className="mt-1 text-zinc-200">{detail.expiresAt ? new Date(detail.expiresAt).toLocaleString("fr-FR") : "Non applicable"}</dd></div>{detail.revokedAt && <><div><dt className="text-zinc-500">Révoquée le</dt><dd className="mt-1 text-zinc-200">{new Date(detail.revokedAt).toLocaleString("fr-FR")}</dd></div><div><dt className="text-zinc-500">Par</dt><dd className="mt-1"><UserCell userId={detail.revokedBy ?? "system"} /></dd></div></>}</dl>{canWrite && detail.status === "active" && !["kick", "clear", "unban", "unwarn"].includes(detail.action) && <div className="mt-5 border-t border-zinc-800 pt-4"><Button variant="danger" size="sm" onClick={() => setToRevoke(detail)}>Révoquer cette sanction</Button></div>}</>}</Drawer>
...
export function SanctionsPage() { const [tab, setTab] = useState("history"); return <div className="space-y-4"><Tabs active={tab} onChange={setTab} tabs={[{ id: "history", label: "Historique" }, { id: "apply", label: "Appliquer une sanction" }, { id: "settings", label: "Paramètres et exemptions" }]} />{tab === "history" ? <History /> : tab === "apply" ? <Apply /> : <Settings />}</div>; }
