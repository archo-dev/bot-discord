  ...
  return <div className="space-y-4">
    <Card title="Historique de modération" description="Toutes les actions (Discord, panel et auto-mod) au même endroit. Filtres, recherche et pagination exécutés côté serveur.">
      <div className="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MemberCombobox guildId={guildId!} value={member} onChange={(id) => { setMember(id); reset(); }} placeholder="Membre concerné…" ariaLabel="Filtrer par membre concerné" />
        <MemberCombobox guildId={guildId!} value={moderator} onChange={(id) => { setModerator(id); reset(); }} placeholder="Modérateur…" ariaLabel="Filtrer par modérateur" />
      ...
      {history.isPending ? <SkeletonList rows={6} /> : history.isError ? <ErrorCard message="Impossible de charger l'historique de modération." onRetry={() => void history.refetch()} /> : (history.data?.items.length ?? 0) === 0 ? <EmptyState icon={<Icon.shield />} title="Aucune action trouvée" description="Les actions appliquées depuis Discord, l'auto-modération ou le panel apparaîtront ici." /> : <>
        ...
        <ul className="space-y-2 md:hidden" aria-label="Actions de modération">
          {history.data!.items.map((item) => (
            <li key={item.id} data-mobile-card>
              <button
                type="button"
                onClick={() => setDetail(item)}
                className="w-full min-w-0 rounded-xl border border-zinc-800 bg-zinc-950/35 p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70"
              >
                <span className="flex min-w-0 items-start justify-between gap-3">
                  <span className="flex min-w-0 items-center gap-2">
                    <ModActionIcon action={item.action} size={32} />
                    <span className="min-w-0">
                      <span className="block font-semibold text-zinc-100">{actionMeta(item.action).label}</span>
                      <span className="mt-0.5 block text-xs text-zinc-500">{sourceLabel(item.source)} · <TimeAgo iso={item.createdAt} /></span>
                    </span>
                  </span>
                  <Badge tone={item.status === "active" ? "success" : item.status === "revoked" ? "neutral" : item.status === "failed" ? "danger" : "warning"}>{statusLabel[item.status]}</Badge>
                </span>
                <span className="mt-3 block min-w-0"><span className="text-xs text-zinc-500">Membre</span>{item.targetId ? <UserCell userId={item.targetId} /> : <span className="block text-zinc-600">—</span>}</span>
                <span className="mt-2 block min-w-0"><span className="text-xs text-zinc-500">Modérateur</span><UserCell userId={item.moderatorId} /></span>
                <span className="mt-2 block break-words text-sm text-zinc-400">{item.reason ?? "Aucune raison"}</span>
              </button>
            </li>
          ))}
        </ul>
...
function Warnings() {
  ...
  return <Card title="Avertissements" description="Chaque avertissement alimente le seuil de warns → mute automatique. Révoquer un warn le retire du décompte actif." action={<div className="w-56 sm:w-64"><MemberCombobox guildId={guildId!} value={userFilter || null} onChange={(id) => setUserFilter(id ?? "")} placeholder="Filtrer par membre…" ariaLabel="Filtrer les avertissements par membre" /></div>}>
    {warnings.isPending ? <SkeletonList rows={4} /> : warnings.isError ? <ErrorCard message="Impossible de charger les avertissements." onRetry={() => void warnings.refetch()} /> : items.length === 0 ? (userFilter ? <EmptyState icon={<Icon.shield />} title="Aucun avertissement pour ce membre" action={<Button variant="secondary" size="sm" onClick={() => setUserFilter("")}>Effacer le filtre</Button>} /> : <EmptyState icon={<Icon.shield />} title="Aucun avertissement" description="Les avertissements donnés avec /warn apparaîtront ici, avec leur statut actif ou révoqué." />) : <>
      ...
      <ul className="space-y-2 md:hidden" aria-label="Avertissements">
        {items.map((warning) => (
          <li key={warning.id} data-mobile-card className={`min-w-0 rounded-xl border border-zinc-800 bg-zinc-950/35 p-3 ${warning.revokedAt ? "opacity-60" : ""}`}>
            <div className="flex min-w-0 items-start justify-between gap-3">
              <div className="min-w-0"><UserCell userId={warning.userId} /><p className="mt-1 text-xs text-zinc-500">Avertissement #{warning.id} · <TimeAgo iso={warning.createdAt} /></p></div>
              {warning.revokedAt ? <Badge tone="neutral">Révoqué</Badge> : <Badge tone="success">Actif</Badge>}
            </div>
            <p className="mt-3 break-words text-sm text-zinc-300">{warning.reason ?? "Aucune raison"}</p>
            <div className="mt-3 flex min-w-0 items-center justify-between gap-3 border-t border-zinc-800 pt-3">
              <span className="min-w-0 text-xs text-zinc-500">Par <UserCell userId={warning.moderatorId} /></span>
              {canWrite && !warning.revokedAt && <Button size="sm" variant="secondary" onClick={() => setToRevoke(warning)}>Révoquer</Button>}
            </div>
          </li>
        ))}
      </ul>
...
function Settings() {
  ...
  return <div className="max-w-5xl space-y-4"><Card title="Rôles exemptés" description="Chaque type de sanction possède sa propre liste. Les rôles supprimés restent signalés afin qu'aucune protection ne disparaisse silencieusement."><div className="mb-4 max-w-md"><Input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Rechercher un rôle…" /></div><div className="hidden overflow-x-auto md:block"><table className="w-full text-sm"><thead><tr className="border-b border-zinc-800 text-left text-xs uppercase tracking-wide text-zinc-500"><th className="py-2 pr-3">Rôle</th>{(["warn", "timeout", "kick", "ban"] as const).map((type) => <th key={type} className="px-2 py-2 text-center">{typeLabel[type]}</th>)}</tr></thead><tbody className="divide-y divide-white/5">{visible.map((role) => <tr key={role.id}><td className="py-2.5 pr-3"><span className="inline-flex items-center gap-2 text-zinc-200"><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: role.color ? `#${role.color.toString(16).padStart(6, "0")}` : "#99aab5" }} />{role.name}<span className="text-xs text-zinc-500">position {role.position}{role.managed ? " · intégration" : ""}</span></span></td>{(["warn", "timeout", "kick", "ban"] as const).map((type) => <td key={type} className="px-2 py-2 text-center"><input aria-label={`${typeLabel[type]} : ${role.name}`} type="checkbox" disabled={!canWrite} checked={value[type].includes(role.id)} onChange={() => toggle(type, role.id)} /></td>)}</tr>)}</tbody></table></div><ul className="space-y-2 md:hidden" aria-label="Rôles exemptés">{visible.map((role) => <li key={role.id} data-mobile-card className="min-w-0 rounded-xl border border-zinc-800 bg-zinc-950/35 p-3"><div className="flex min-w-0 items-start gap-2"><span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: role.color ? `#${role.color.toString(16).padStart(6, "0")}` : "#99aab5" }} /><span className="min-w-0 break-words font-medium text-zinc-200">{role.name}<span className="mt-0.5 block text-xs text-zinc-500">Position {role.position}{role.managed ? " · intégration" : ""}</span></span></div><div className="mt-3 grid grid-cols-2 gap-2">{(["warn", "timeout", "kick", "ban"] as const).map((type) => <label key={type} className="flex min-h-10 items-center justify-between gap-2 rounded-lg border border-zinc-800 px-2.5 text-xs text-zinc-300"><span>{typeLabel[type]}</span><input className="h-5 w-5" aria-label={`${typeLabel[type]} : ${role.name}`} type="checkbox" disabled={!canWrite} checked={value[type].includes(role.id)} onChange={() => toggle(type, role.id)} /></label>)}</div></li>)}</ul>{(["warn", "timeout", "kick", "ban"] as const).flatMap((type) => value[type].filter((id) => !knownRoles.has(id)).map((id) => <p key={`${type}-${id}`} className="mt-2 break-all text-sm text-amber-300">Rôle introuvable ({id}) encore exempté pour {typeLabel[type]}.</p>))}<div className="mt-5 flex justify-end"><Button className="w-full sm:w-auto" disabled={!canWrite || save.isPending} onClick={() => save.mutate()}>Enregistrer les exemptions</Button></div></Card><InfoCard icon={<Icon.shield />} title="Protections prioritaires">Les exemptions ne remplacent jamais les protections Discord : propriétaire, auto-sanction, hiérarchie, permissions du bot et de l'auteur restent vérifiés côté serveur.</InfoCard></div>;
}
